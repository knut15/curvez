...
- **새 색을 만들지 않는다.** 색은 `apps/handwork/src/app/globals.css` 에서 이미 확정됐다
  - **이유:** 색은 배경과의 쌍으로만 의미가 있고, 그 쌍 20개가 이미 대비 실측을 통과한 상태다. 새 색을 하나 들이면 그 색과 기존 배경 전부의 조합을 다시 재야 하는데, 그 재검사가 어디에도 기록되지 않은 채 "통과" 로 남는다
...
"정본은 `apps/handwork/src/app/globals.css` 다. 이 표는 사본이고, 값이 갈리면 CSS 가 이긴다" 라고
...
| 간격·타이포 값이 갈린다                                          | **Tailwind 4 기본 스케일을 쓴다.** 새 CSS 변수를 만들지 않는다                                                                                                     | `tokens.md` 의 `## 간격·타이포는 새 토큰을 만들지 않는다` 가 이미 그렇게 정했다                        |
...
살아 있는 예시이고, 새 스펙은 그것과 같은 모양으로 쓴다. 아래는 그 문서들에서 실측한 필수 요소다.
...
2. 그래도 갈리면 **접근성 기준을 지키는 쪽**을 고른다 (본문 대비 4.5:1, disabled 3:1, 타깃 24x24px)
...
**받는 쪽:** `curvez-nextjs` 의 화면 구현 실측 — 어느 파일 어느 줄에 어떤 클래스 문자열이 반복되는지,
...
- **선행:** `curvez-nextjs` (화면 구현 실측 — 어느 클래스 문자열이 몇 번 반복되는지). 디자인 값의 선행은 없다 — 이 담당이 값의 출발점이다
...
- **파일 소유권:** `apps/handwork/design/` **아래만** 쓴다. 그리고 `.curvez/handoff/handwork-design-system.<timestamp>.json` 하나를 쓴다
  ...
  - **이유:** `curvez-nextjs` 가 `${paths.web}` = `apps/handwork` 를 통째로 소유한다. `design/` 을 갈라 쓰는 예외는 `.curvez/team.md` 에 선언돼 있고, 오케스트레이터는 팀을 짤 때 그 표를 먼저 읽는다. 표 밖으로 나가면 예외가 무효가 되고, 병렬 실행에서 나중에 쓴 쪽이 앞선 쪽을 조용히 지운다
...
#    (출력 줄 수가 곧 누락 건수. 키는 기존 12개 문서에서 실측한 것이다)
...
# 5. 대비 실측. tokens.md 끝의 `## 대비 검증` 줄을 파싱한다
SYS="$SYS" node -e '
const fs=require("fs");
