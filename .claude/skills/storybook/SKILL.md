...
exit 0 으로 끝나는 것까지 실측으로 확인된 값이다. **이 값을 다시 고르지 마라.**
...
exit 0 이어야 한다. 실패하면 아래 순서로 가른다.

| 증상                                  | 먼저 볼 것                                                                              |
| ------------------------------------- | --------------------------------------------------------------------------------------- |
| 스타일이 전혀 안 먹는다               | `preview.ts` 의 `../src/app/globals.css` import 경로                                    |
| 글자만 다르다                         | `preview.css` 의 Pretendard import                                                      |
| `react-remove-scroll is not in cache` | `node --version` 을 실측해 보고에 남긴다. Node 20 에서 보고된 사례다                    |
...
- **새 프리미티브는 `shadcn add` 로 받는다.** 이 저장소의 shadcn 은 `base-nova` 스타일이고 기반은 `@base-ui/react` 다 — Radix 가 아니다.
  `pnpm exec shadcn add button --dry-run` 이 `src/shared/ui/button.tsx` 로 해석되고, 받는 소스가 `@base-ui/react/button` + `cva` + `cn` 을 쓰는 것까지 실측으로 확인됐다
...
- **확인하지 않은 수치를 적지 마라.** 스토리 개수·빌드 시간·Node 판본은 전부 명령을 돌려 얻은 출력을 옮긴다
  - **이유:** 그럴듯한 수치는 실측한 수치와 보고서에서 똑같이 생겼다. 읽는 사람은 둘을 구분하지 못하고 믿는다
