...
- state:empty — MDX 가 0건일 때. list 자리에 문구 "아직 공개한 케이스가 없습니다." 한 줄과 홈으로 돌아가는 링크 1개. 필터가 없으므로 "검색 결과 없음" 과 구분할 필요가 없다
- state:error — 없다. MDX 파싱 실패는 빌드 실패로 끝나고 배포되지 않는다

## responsive

- nextjs: <768 1열 · py-10 / >=768 2열 · py-16, 컨테이너 max-w-5xl
