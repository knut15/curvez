...
여기서 갈리는 질문이 하나 있다. 요청이 아예 안 나간 것인가, 나갔는데 서버가 거절한 것인가. 이 답은 코드가 아니라 브라우저가 들고 있다.

이 글은 F12 로 여는 Network 탭에서 Headers 와 Payload 를 어떤 순서로 읽는지를 정리한 것이다. 브라우저 개발자 도구를 처음 열어 보는 개발자를 대상으로 한다. HTTP 를 깊이 알지 못해도 읽을 수 있게 정리했다.
...
여기가 제일 헷갈리는 자리다. Payload 와 Response 가 둘 다 JSON 을 보여주니 어느 쪽이 뭔지 매번 다시 확인하게 된다. 기준은 하나다. **Payload 는 브라우저가 보낸 것이고, Response 는 서버가 돌려준 것이다.**
