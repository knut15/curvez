...
![가로 막대가 세로로 여러 개 쌓여 있고 대부분 흐린 회색이다. 그중 셋만 진하게 남았고, 하나는 오른쪽으로 끌려 나와 폭이 다른 두 덩어리로 갈라져 있다.](/blog/b1-devtools.jpg)

로그인 버튼을 눌렀는데 화면이 그대로였다. 코드를 몇 번을 다시 읽어도 이상한 곳이 없었고, 콘솔에도 빨간 줄 하나 안 떴다. 눌렀다는 사실 말고는 아무 흔적이 없었다.

그래서 궁금한 게 하나 생겼다. 요청이 아예 안 나간 걸까, 나갔는데 서버가 거절한 걸까?

그때 처음으로 F12 를 눌러 Network 탭을 열었다. 이 글은 그 탭에 뜨는 Headers 와 Payload 를 어떻게 읽는지에 대한 이야기다.

## 1. 목록에 줄이 수십 개 뜨는데 뭔지 몰랐다

Network 탭을 열어 두고 새로고침을 했더니 줄이 수십 개 쏟아졌다. 이미지, 폰트, 자바스크립트 파일, 그리고 그 사이 어딘가에 내가 찾던 로그인 요청이 있었다. 어디서부터 봐야 할지 몰라서 그날은 그냥 다시 닫았다.
...
내가 보고 싶었던 건 API 요청 하나뿐이었다. 목록 위쪽 필터에서 `Fetch/XHR` 만 켜면 이미지와 폰트가 통째로 빠지고 API 만 남는다. 이거 하나 알고 나니 수십 줄이던 목록이 서너 줄이 됐다.

## 2. 보낸 것과 받은 것이 계속 헷갈렸다

줄 하나를 클릭하면 오른쪽에 탭이 또 뜬다. Headers · Payload · Preview · Response · Initiator · Timing · Cookies.

여기서 오래 헤맸다. Payload 와 Response 가 둘 다 JSON 을 보여주니까 어느 쪽이 뭔지 매번 다시 확인해야 했다. 나중에야 기준이 하나뿐이라는 걸 알았다. **Payload 는 내가 보낸 것이고, Response 는 서버가 돌려준 것이다.** 이 한 줄을 몰라서 한참 돌아갔다.
...
## 4. 그래서 내가 보낸 데이터는 어디에 있나

Payload 탭이다. 다만 요청 방식에 따라 표시되는 모양이 다르다. 처음 볼 때 이 차이를 몰라서 "왜 어떤 요청은 Payload 탭이 아예 없지" 하고 한동안 이상하게 생각했다.
...
Payload 탭이 안 보인다고 뭐가 잘못된 건 아니에요. GET 에는 본문이 없어서 쿼리스트링만 뜨고, 본문이 아예 없는 요청은 Payload 탭 자체가 생기지 않아요.

## 5. 로그인 요청 하나를 통째로 펼쳐 보면

지금까지 나눠 본 것들이 실제로는 한 화면에 같이 있다. 로그인 요청 하나를 펼치면 이런 모양이다.
...
이 한 화면에 네 가지가 다 들어 있다. 내가 무엇을 보냈는지는 Payload 에, 인증 정보가 같이 실렸는지는 `Cookie` 에, 서버가 새 토큰을 심었는지는 `Set-Cookie` 에, 무엇을 돌려줬는지는 Response 에 있다.
...
한동안 안 건드리고 있었는데, 알고 보니 여기 있는 것들이 디버깅 시간을 제일 많이 줄여 줬다.
...
지금은 화면이 이상하면 코드부터 다시 읽지 않는다. F12 를 누르고 `Fetch/XHR` 필터를 켠 다음, Status 를 보고, Request Headers 에 인증 정보가 실렸는지를 본다. 코드에 `console.log` 를 심어 가며 찾던 것들이 대부분 여기 이미 적혀 있었다.

혹시 지금도 로그인 버튼을 누르고 화면만 보고 있다면, 일단 F12 부터 눌러 보세요. 코드에 없는 답이 거기 적혀 있을 때가 많아요.
