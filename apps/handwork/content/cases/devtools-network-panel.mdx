export const meta = {
  title: "Network 탭으로 로그인 요청 오류 확인하기",
  summary: "요청 목록과 상태 코드를 확인한 뒤 Headers와 Payload를 읽는다. 로그인 요청을 예로 들어 인증 정보와 전송 데이터를 찾는 순서를 설명한다.",
  date: "2026-07",
  role: "디버깅",
...
우선 Network 탭에서 요청이 전송됐는지 확인한다. 요청이 있다면 서버가 어떤 상태 코드로 응답했는지 본다.
...
목록에 줄이 없으면 안 나간 것이고, 줄이 있는데 `401` 이면 나갔는데 거절당한 것이다.

화면이 예상대로 동작하지 않을 때는 요청 기록부터 확인해 볼 수 있다. F12 를 누르고 `Fetch/XHR` 필터를 켠 다음, Status 를 보고, Request Headers 에 인증 정보가 실렸는지를 본다. 코드에 `console.log` 를 심어 가며 찾는 값이 대부분 여기 이미 적혀 있다.

## 관련 글

- [쿠키와 세션으로 로그인 상태를 유지하는 원리](/cases/session-cookie-login)
- 인증·인가 & Bearer 헤더 방식
- [CDN 캐시 때문에 이전 이미지가 보이는 이유](/cases/cdn-edge-delivery)
