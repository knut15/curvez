...
디자인 값이 밖의 문서에 매여 있으면, 그 문서가 바뀔 때 화면이 바뀌었는지 아닌지 알 방법이 없다.
그래서 여기 둔다.
...
근거는 화면에 반복해 나온 중복이다. 괄호 안은 같은 클래스 문자열이 반복된 횟수.
...
굵은 글씨 둘은 스펙과 구현이 다른 곳이다. 무엇이 왜 달라졌는지는
...
| 대비 검증        | 20쌍(라이트 10 · 다크 10), 실패 0. 렌더 화면에서 쟀다           |               20 |
