...
| variant  | `inline` \| `bare` | X    | `inline` | `inline`=밑줄 있음(4회), `bare`=밑줄 없음(2회) |
...
**이유:** 실측이 두 형태뿐이다. 밑줄이 있거나 없거나이고, 그 사이에 중간값이 없다.

**`external` prop 을 두지 않는다.**
**이유:** 실측에서 외부 링크가 0건이다. 생기면 그때 `target`·`rel`·아이콘을 한꺼번에 정한다 —
...
**hover 에서 위치를 옮기지 마라.** `translate`·`scale` 실측 0건이다.
...
- `bare` 로 쓰이는 이웃 링크는 좌우로 갈라 배치한다 — 왼쪽 `←` 이전 글, 오른쪽 `다음 글 →`. `flex justify-between gap-4`(`case-detail.tsx:52`). 한쪽이 없으면 빈 `<span />` 이 자리를 지킨다(`case-detail.tsx:62`)
