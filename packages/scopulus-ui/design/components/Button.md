...
```bash
grep -rn '<Button\b\|buttonClass(' apps/*/src packages/scopulus-ui/src --include='*.tsx' | grep -v stories
```

| 파일                                       | 줄  | 어떻게                                      |
| ------------------------------------------ | --- | ------------------------------------------- |
| `packages/scopulus-ui/src/ui/dialog.tsx`   | 64  | 닫기 버튼. `variant="ghost" size="icon-sm"` |
| `packages/scopulus-ui/src/ui/dialog.tsx`   | 109 | 바닥 버튼. `variant="outline"`              |
| `packages/scopulus-ui/src/ui/app-link.tsx` | 51  | `cta` 가 `buttonClass` 를 입는다            |

**처음에는 0곳이었다.** `ThemeToggle` 이 `<button>` 을 직접 들고 있어서 이 컴포넌트를 쓰는 곳이
없었고, 그 근거로 `outline` 과 `icon-sm` 을 뺐다. `Dialog` 가 둘 다 쓰면서 그 근거가 사실이
아니게 됐고 되살렸다(2026-09-11).

**variant 와 size 를 지어내지 않는다.** shadcn 이 정한 이름
(`default` / `secondary` / `outline` / `ghost` / `link`, `sm` / `default` / `lg` / `icon`)에서
실제로 쓰는 것만 남기고, 나머지는 아래 "넣지 않는 값" 에 이유와 함께 적었다.
...
| size    | `default` | 높이 40px(`h-10`), 좌우 16px(`px-4`), `text-sm font-medium` | 40px·16px 둘 다 화면에 나온 스케일 안이다(step 10 ×7, step 4 ×13). 헤더 아이콘 버튼과 같은 높이라 나란히 놓여도 밑선이 맞는다 |
| size    | `icon`    | 40x40(`size-10`)                                            | `shared/ui/theme-toggle.tsx:21` 에서 쓰던 것. [`SiteHeader.md`](SiteHeader.md) 의 `a11y:target` 이 "토글 40x40" 으로 기록했다 |
| 반경    | 공통      | `rounded-md`(8px)                                           | `shared/ui/theme-toggle.tsx:21` 에서 쓰던 것. `../tokens.md` `## 형태`: "아이콘 버튼 · 포커스 링 모서리 = `rounded-md`"       |
...
| variant `link`      | 그 자리는 `AppLink` 가 맡는다. `variant="link"` 는 `<button>` 을 링크처럼 보이게 만드는 것이라, 주소가 바뀌는 것과 바뀌지 않는 것의 구분을 화면에서 지운다. 새 탭 열기와 주소 복사가 막히는 것도 같은 문제다. **반대 방향은 막지 않는다** — `<a>` 를 버튼처럼 보이게 하는 것은 두 기능이 그대로 살아 있어서, `AppLink` 의 `cta` 가 `buttonClass` 를 입는 방식으로 연다 |
| size `sm`           | 이 사이트에 버튼이 들어갈 자리가 둘뿐이다 — 헤더의 아이콘 하나와, 아직 없는 CTA 하나. 크기 단계가 둘 이상 필요한 자리가 0곳이다. 화면에 `<button>` 이 1번 나온 것이 그 근거다                                                                                                                                                                                          |
...
`buttonClass` 를 내보낸다. 랜딩의 주 행동 두 자리가 이 클래스를 입는다.

- 요소는 `<a>` 로 둔다. `Button` 에 `<a>` 를 넣지 마라 — Base UI 의 `Button` 은 언제나
  `role="button"` 을 붙이고, 그러면 링크 역할이 덮여 새 탭 열기와 주소 복사가 막힌다.
  shadcn 문서가 같은 이유로 링크에는 `buttonVariants()` 를 쓰라고 적어 뒀다
- 쓰는 쪽은 `AppLink` 의 `variant="cta"`(주) 와 `variant="cta-quiet"`(그 옆)다.
  앱이 `buttonClass` 를 직접 부르지 않는다 — 이동은 `AppLink` 하나로 모은다
- 클래스를 복사해 쓰지 마라. 두 벌이 되면 한쪽만 고쳐질 때 모양이 달라진다
...
| focus-visible | 키보드 포커스   | `focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring`. 화면에 나온 8곳이 전부 이 세 조각이다. `focus-visible:rounded-sm` 은 붙이지 않는다 — 이미 `rounded-md` 를 가졌다              |
| pressed       | `:active`       | `active:scale-97` + `motion-reduce:active:scale-100`. `shared/ui/theme-toggle.tsx:21` 에서 쓰던 것. 전이 속성에 `transform` 을 더한다: `transition-[color,background-color,transform]`                             |
...
- a11y:label — `size="icon"` 이면 `aria-label` **필수**다. 라벨 원문은 부르는 쪽이 정한다(`ThemeToggle` 은 "테마 전환" — `shared/ui/theme-toggle.tsx:11`). `size="default"` 는 라벨이 보이므로 `aria-label` 을 중복 지정하지 마라 — 이유: 보이는 글자와 읽히는 글자가 달라지면 음성 조작이 실패한다
- a11y:focus — 포커스 순서는 DOM 순서와 같다. `disabled` 버튼은 포커스를 받지 않는다. 누른 뒤에도 포커스를 잃지 않는다 — 이유: 토글처럼 제자리에서 상태가 뒤집히는 버튼은 연속으로 누르는 일이 있다
- a11y:contrast — `default` 기본 fg=`--primary-foreground` / bg=`--primary` 라이트 15.82 · 다크 16.13 (`../tokens.md` 의 "CTA 라벨/CTA 배경" 대비값). `ghost` 기본 fg=`--muted-foreground` / bg=`--background` 라이트 5.28 · 다크 7.97, hover 는 fg=`--accent-foreground` / bg=`--accent`. `disabled` 는 3:1 이상이면 통과다(WCAG 1.4.11)
...
- 모든 폭에서 같은 크기다. 브레이크포인트 분기를 넣지 마라 — 이유: 화면에 나온 버튼 1곳에 분기가 0건이고, 40px 은 375px 화면에서도 24x24 최소를 여유 있게 넘는다
...
- `size="icon"` 안의 아이콘은 20px(`size-5`)이다. `shared/ui/theme-toggle.tsx:25-26` 에서 쓰던 것. 40px 상자 안에서 상하좌우 10px 씩 남는다
