...
**이유:** 두 사용처가 그 세 클래스도 같다. 밖에 두면 세 번째 본문 화면이 생길 때 폭이 달라진다.
...
**`h4` 이하를 정하지 않는다.** 화면에 0번 나온다.
**이유:** 지금 쓰이지 않는 깊이의 값을 정하면 검증되지 않은 값이 남는다. 본문에 h4 가 필요해지면
그때 이 표에 한 줄을 더한다.

**`table`·`img`·`hr` 서식을 정하지 않는다.** 화면에 0번 나온다. 같은 이유다.
...
| focus-visible | 본문 안 링크에 키보드 포커스 | 링크의 포커스 링. **`prose.ts:8` 의 Prose 문자열에는 focus 규칙이 없다.** MDX 링크가 `AppLink` 를 거치지 않고 raw `<a>` 로 렌더되기 때문이다 |
...
`prose.ts:8` 은 링크의 **hover** 만 정의하고 **focus-visible** 은 정의하지 않는다.
...
- 상세 화면의 머리(h1 · 메타 · 태그)도 같은 `max-w-[68ch]` 를 쓴다(`case-detail.tsx:25`, `lab-detail.tsx:13`). 두 폭이 서로 다르면 제목과 본문의 왼쪽 끝이 어긋난다
