...
[`../tokens.md`](../tokens.md) 가 그 둘을 실측 9건씩으로 정했고, 값이 없으면 브라우저 기본값이 쓰여
같은 화면의 다른 오버레이와 속도가 갈린다.

## a11y

- a11y:label — **툴팁의 글자가 트리거의 접근 이름이 된다.** Base UI 가 `aria-describedby` 또는 `aria-labelledby` 로 묶는다. **트리거에 `aria-label` 을 따로 주고 툴팁에 다른 글자를 넣지 마라 — 이유:** 보이는 글자와 읽히는 글자가 갈려 음성 조작이 실패한다. 아이콘 버튼에 툴팁을 달 때는 **둘 중 하나만 쓴다** — [`Button.md`](Button.md) 가 `size="icon"` 에 `aria-label` 을 필수로 정했으므로, 툴팁을 달면 그 `aria-label` 과 툴팁 글자를 **같은 문자열로** 맞춘다. [`ThemeToggle.md`](ThemeToggle.md) 의 "테마 전환" 이 그 예다
...
- a11y:contrast — fg=`--background` / bg=`--foreground` 다. 두 토큰의 값은 `--primary-foreground` · `--primary` 와 **같다** (`../../src/tokens.css:64,71`·`:65,70` 라이트 · `:99,106`·`:100,105` 다크). 그래서 [`../tokens.md`](../tokens.md) `## 대비 검증` 의 "CTA 라벨/CTA 배경" 실측값이 그대로 적용된다 — **라이트 15.82 · 다크 16.13**, 4.5:1 통과. **글자가 `text-xs`(12px)이라 큰 글자 완화(3:1)를 받지 않는다** — 4.5:1 기준을 그대로 받고, 통과한다. 화살표는 면과 같은 색이라 대비 대상이 아니다
