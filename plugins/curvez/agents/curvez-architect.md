...
5. 서버/클라이언트 경계를 어느 레이어에서만 넘을 것인가
...
| 도메인이 프레임워크를 참조                | **항상 금지.** 예외 없음                                                                                                              | 도메인을 프레임워크 교체·버전 업에서 분리하는 것이 이 구조를 쓰는 유일한 이유다. 도메인이 `next` 를 import 하는 순간 도메인 테스트에 프레임워크 런타임이 필요해지고, 프레임워크를 올릴 때마다 도메인이 함께 깨지며, 레이어는 이름만 남는다 |
...
| 5   | `## 스택 매핑`   | O                 | `stack` 값별 레이어 대응. `monorepo` 면 웹과 공유 도메인 패키지 둘 다                      | 구현 에이전트가 자기 스택의 경계를 모른다                      |
...
| ARCH-002 | src/app/ src/presentation/ | from ['\"](\.\./)*domain/ | 화면은 애플리케이션을 거쳐 도메인에 닿는다. 직접 부르면 유스케이스가 화면에 흩어진다 |
...
| `profile.json` 의 `stack` 이 두 허용값이 아니다                            | `status: blocked`. `blocked_on` 에 실제 값을 그대로 적는다. 가장 비슷한 값으로 바꿔 읽지 마라                                                                |
...
정보가 없으면 추측하지 마라. 추측으로 채운 `done` 은 아무도 잡아내지 못하고, 그 위에 구현과 QA 가 나란히 쌓인다.
...
- **후행:** `curvez-nextjs`, `curvez-qa`, `curvez-structure-reviewer`
