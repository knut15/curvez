...
|---|---|
| "커밋해줘" | 커밋까지 |
| "푸시해줘" | 푸시까지 (실행은 사용자가 한다 — 아래 "가드가 막는 것") |
...
| 차단되는 것 | 가드가 대는 이유 |
|---|---|
| `git push` (전부) | 원격에 반영되면 되돌리기가 로컬 작업보다 훨씬 비싸다 |
...
```
아래 명령을 직접 실행해 주십시오. (curvez 가드가 push 를 차단합니다)

  git push -u origin feature/attendance-calendar

실행 뒤 알려 주시면 PR 생성으로 이어가겠습니다.
```

즉 **`git push` 이후 단계(PR 생성·CI 확인·머지)는 사용자가 push 를 실행한 뒤에야 시작된다.**
push 없이 `gh pr create` 를 시도하지 마라 — 원격에 브랜치가 없어 실패한다.
...
- **만들지 못한 것을 적지 않는다.** push 를 사용자가 아직 실행하지 않았으면 PR 은 존재하지 않고,
  따라서 `artifacts` 에도 없다
...
| `user` | 가드가 막은 명령의 전문 (`git push …`) 과 왜 사람이 실행해야 하는지 | 차단된 즉시. 다음 단계로 넘어가기 전 |
...
| `git push` 가 필요하다 | 이 에이전트는 실행할 수 없다. 명령 전문을 제시하고 사용자 실행을 요청한 뒤 **멈춘다.** `status` 는 `partial`, `blocked_on` 에 `who: user` 로 남긴다 |
...
# 7. 원격 반영 여부. push 는 가드가 막으므로 사용자가 실행한 뒤 여기서 확인한다.
git status -sb
