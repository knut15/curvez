...
- **소스 코드 수정** — `curvez-nextjs` 가 한다. 커밋하려는 변경이
...
| `.curvez/handoff/curvez-nextjs.*.json`                           | 조건부 | 변경 파일 목록의 출처. 없으면 `git status --short` 로 확인한 것만 담는다                                                                                                                    |
...
| `curvez-nextjs`       | 커밋하려는 변경이 게이트(typecheck·lint)에서 깨졌을 때 실패 출력 원문                                | 확인한 즉시. 커밋하기 전                          |
...
- **선행:** `curvez-nextjs` (구현 완료), `curvez-qa` (검증 완료),
