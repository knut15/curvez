...
에이전트 12/12 · 스킬 16/16 통과, exit 0 이면 된다.
...
설치하면 에이전트 12종이 서브에이전트 이름으로 등록되고, 스킬 16종이 `curvez:` 네임스페이스로 붙는다.
...
| 스킬 16종     | `plugins/curvez/skills/*/SKILL.md` | 무엇을 순서대로 하는가. 실행 중인 에이전트가 읽는 절차서   |
...
순차로 강등한다. 예를 들어 모노레포의 공유 도메인 패키지를 건드리는 라운드에서는 그 경로를 읽는
에이전트를 동시에 띄우지 않는다.

이 그림은 표준형이지 고정된 파이프라인이 아니다. 지시가 이미 한 문장으로 굳어 있으면 R1 을 건너뛰고,
조사할 것이 없으면 R1 이 한 명으로 줄어든다. 라운드 구성과 병렬 여부는 매번 오케스트레이터가 판정해
...
| 스킬 16종     | 있음 (`plugins/curvez/skills/`)                                                                                               |
...
| 프리셋        | 있음 — 아키텍처 `ddd` 1종, 스택 2종 (`nextjs` `monorepo`)                                                                     |
| 훅            | 있음 — `guard-bash` `guard-forbidden-words` `validate-on-write` `check-handoff` `notify-update`. `hooks/hooks.json` 으로 등록 |
...
넓힐 때는 오탐도 함께 확인한다. 실제로 "구현 에이전트 2종", "에이전트**와** 16종 스킬" 을
