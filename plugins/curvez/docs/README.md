...
curvez 는 **보일러플레이트**다. 여기 들어 있는 13종 에이전트와 17종 스킬은 어느 프로젝트에나
...
에이전트 13/13 · 스킬 17/17 통과, exit 0 이면 된다.
...
설치하면 에이전트 13종이 서브에이전트 이름으로 등록되고, 스킬 17종이 `curvez:` 네임스페이스로 붙는다.
...
| 스킬 17종     | `plugins/curvez/skills/*/SKILL.md` | 무엇을 순서대로 하는가. 실행 중인 에이전트가 읽는 절차서   |
...
| 스킬 17종     | 있음 (`plugins/curvez/skills/`)                                                                                               |
...
넓힐 때는 오탐도 함께 확인한다. 실제로 "구현 에이전트 2종", "에이전트**와** 17종 스킬" 을
