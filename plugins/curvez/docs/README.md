...
curvez 는 **보일러플레이트**다. 여기 들어 있는 12종 에이전트와 15종 스킬은 어느 프로젝트에나
...
| 층 | 위치 | 무엇인가 |
|---|---|---|
| 에이전트 12종 | `plugins/curvez/agents/*.md` | 누가 무엇을 책임지는가. 역할·도구 권한·소유 경로·통신 상대 |
...
| 항목 | 상태 |
|---|---|
| 에이전트 12종 | 있음 (`plugins/curvez/agents/`) |
...
실제로 에이전트가 12종이 된 뒤에도 README 는 "11종" 이라고 말했고, 실행기 넷이 생긴 뒤에도
