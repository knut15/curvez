...
문서만 바뀐다. 동작·스키마·라인업 변경 없다.

| #   | 변경                                                                                                                                                                                     | 정본                                 |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| ①   | **`templates/CLAUDE.md` 의 출처 표기에 원본 버전·복사 시점·라이선스를 붙였다.** 전에는 스킬 이름만 있어 원본이 개정돼도 사본이 낡았는지 대조할 수 없었다                                 | `templates/CLAUDE.md` 첫머리 주석    |
| ②   | **차용 범위를 명시했다.** 1~~4절만 원본(`karpathy-guidelines` 1.0.0)의 한글 번역이고 5~~12절은 대응이 없는 자체 작성분이다. 한 줄짜리 출처 표기가 어디까지가 남의 것인지를 가리고 있었다 | 같은 주석                            |
| ③   | **`VENDOR.md` 에 이 사본을 등재했다.** `vendor/` 밖에 있는 유일한 차용 자산이라 목록에서 빠져 있었다                                                                                     | `vendor/mattpocock-skills/VENDOR.md` |

`templates/CLAUDE.md` 를 `vendor/` 로 옮기지 않은 이유는 그 파일이 **스캐폴드 산출물**이기 때문이다.
bootstrap 이 새 프로젝트에 복제하므로 템플릿 자리에 있어야 한다.

### 업데이트 절차

절차의 정본은 [README 의 업데이트](README.md#업데이트)다. 끝나면 `installed_plugins.json` 의
`curvez@curvez` 가 `version: "0.4.1"` 인지 확인한다.

### 프로젝트에서 할 일

- **없다.** 이미 스캐폴드된 `CLAUDE.md` 는 bootstrap 이 덮지 않는다. 출처 주석을 받고 싶으면
  `plugins/curvez/templates/CLAUDE.md` 첫머리 주석을 직접 옮겨 붙인다
