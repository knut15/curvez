...
- 토큰이 **움직일 때** 무엇이 맞는지(이징·지속 시간·중단 가능성)를 정할 때 → `motion-standards` 를 쓴다. 이 스킬은 정지 상태의 값만 확정한다
- 디자인 작업을 어느 스킬이 맡을지부터 갈릴 때 → `design-routing` 을 먼저 쓴다. 담당이 정해진 뒤에 이 스킬로 값을 확정한다
